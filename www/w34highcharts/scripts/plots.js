...
function plot_js(units, ptype, span, plt_div, dplots = false, cdates = false, reload_plot_type_span = null, realtime = false, radial = false){
	...
	var plotsnoswitch = ['rainmonthplot','lightningmonthplot','windroseplot','windrosegustplot','bartempwindplot','windbarplot'];
	...
	*****************************************************************************/
	...
	function create_tempderived_chart(options, span, seriesData, units){
	    if (do_radial){
	        ...
	        try{
	            dataMinMax.push(convert_temp(seriesData[0].windchillplot.units, "C", reinflate_time(seriesData[0].windchillplot.appTempminmax)));
	            dataAvg.push(convert_temp(seriesData[0].windchillplot.units, "C", reinflate_time(seriesData[0].windchillplot.appTempaverage)));
	        }catch{}
	        return do_radial_chart(options, dataMinMax, dataAvg, ['Windchill', 'Heatindex', 'Apparent'], tempcolors);
	    }
	    else if (span[0] == "yearly"){
	...
	function create_apparent_chart(options, span, seriesData, units){
	    if (do_radial){
	        ...
	        try{
	            dataMinMax.push(convert_temp(seriesData[0].windchillplot.units, "C", reinflate_time(seriesData[0].windchillplot.appTempminmax)));
	            dataAvg.push(convert_temp(seriesData[0].windchillplot.units, "C", reinflate_time(seriesData[0].windchillplot.appTempaverage)));
	            return do_radial_chart(options, dataMinMax, dataAvg, ['Apparent'], tempcolors);
	        }catch(err){
	            console.log(err);
	            return null;
	       }
